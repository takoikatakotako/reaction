...
export async function fetchNotices(): Promise<entity.Notice[]> {
  const response = await fetch(apiBaseUrl + '/api/notice/list');
  if (!response.ok) {
    throw new Error('お知らせ一覧の取得に失敗しました。');
  }
  const noticeList: entity.NoticeList = await response.json();
...
export async function fetchNotice(id: string): Promise<entity.Notice> {
  const response = await fetch(apiBaseUrl + `/api/notice/detail/${id}`);
  if (!response.ok) {
    throw new Error('お知らせの取得に失敗しました。');
  }
  const notice: entity.Notice = await response.json();
  return notice;
}
...
export async function addNotice(addNotice: entity.AddNotice) {
  const response = await fetch(apiBaseUrl + '/api/notice/add', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${apiKey}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify(addNotice),
  });
...
export async function editNotice(editNotice: entity.EditNotice) {
  const response = await fetch(apiBaseUrl + '/api/notice/edit', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${apiKey}`,
      'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    throw new Error('お知らせの編集に失敗しました。');
  }
}
...
export async function deleteNotice(id: string) {
  const response = await fetch(apiBaseUrl + '/api/notice/delete', {
    method: 'DELETE',
    headers: {
      Authorization: `Bearer ${apiKey}`,
      'Content-Type': 'application/json',
